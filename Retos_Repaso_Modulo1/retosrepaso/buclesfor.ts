...
//3//
let myArr: string[] = ["un", "dos", "tres"];

function myRevert(myArr: string[]): string[] {
    let arrayReves: string[] = [];
    for (let i = myArr.length - 1; i >= 0; i--) {
        arrayReves.push(myArr[i]);
    }
    return arrayReves;
}
let arrayReves: string[] = myRevert(myArr);
...
function isRainbow(colors: string[]): void {
    let rainbowColores = ["rojo", "naranja", "amarillo", "verde", "cian", "azul", "violeta"]; 
    for (let i = 0; i < colors.length; i++) {
...
export function add(myWords: string[]): number {
    let caracteresFrase = 0; // Declarar e inicializar la variable caracteresFrase
...
let myWords: string[] = ["hola, que tal, estoy en la biblioteca", "hola", "que tal"];
